...
class SearchHandler extends Handler {
	...
	function search($args, &$request) {
		...
		//var_dump( count($results->theArray) );
		...
		$templateMgr->assign('resultsInCurrentPage', count($results->theArray));
		$resultsInCurrentPage = count($results->theArray);
		$total = $results->count;
		$page = $results->page;
		$resultsPerPage = $results->itemsPerPage;
		$start = ($page-1)*$resultsPerPage;
		$end = $start + $resultsInCurrentPage;
		++$start;
		$templateMgr->assign('pagination',"Showing results $start to $end of $total");
		
		$this->_assignSearchFilters($request, $templateMgr, $searchFilters);
		
		//echo "From search function";
		//echo "<pre>";
		//print_r($results);
		//echo "</pre>";
