...
class SubmitHandler extends AuthorHandler
{
    ...
    */
    function saveSubmit($args, $request)
    {
        ...
        $submitForm = new $formClass($article, $journal, $request);
        $submitForm->readInputData();
        if (!HookRegistry::call('SubmitHandler::saveSubmit', array(
            $step,
            &$article,
            &$submitForm
        ))) {
            ...
            if (!isset($editData) && $submitForm->validate()) {
                ...
                HookRegistry::call('Author::SubmitHandler::saveSubmit', array(
                    &$step,
                    &$article,
                    &$submitForm
                ));
                if ($step == 5) {
                    ...
                    $request->redirect(null, null, 'submit', $step + 1, array(
                        'articleId' => $articleId
                    ));
                }
            } else {
    ...
    */
    function submitUploadSuppFile($args, $request)
    {
        ...
        $submitForm->setData('title', array(
            $article->getLocale() => __('common.untitled')
        ));
        $suppFileId = $submitForm->execute();
        $request->redirect(null, null, 'submitSuppFile', $suppFileId, array(
            'articleId' => $articleId
        ));
    }
    /**    
    ...
    */
    function saveSubmitSuppFile($args, $request)
    {
        ...
        if ($submitForm->validate()) {
            $submitForm->execute();
            $request->redirect(null, null, 'submit', '4', array(
                'articleId' => $articleId
            ));
        } else {
            $submitForm->display();
    ...
    */
    function expediteSubmission($args, $request)
    {
        ...
        if (Validation::isEditor($journal->getId()) && $article->getSubmissionFileId()) {
            ...
            $request->redirect(null, 'editor', 'submissionEditing', array(
                $article->getId()
            ));
        }
    ...
    */
    function validate($request, $articleId = null, $step = false, $reason = null)
    {
        ...
        if ($step !== false && ($step < 1 || $step > 5 || (!$articleId && $step != 1))) {
            $request->redirect(null, null, 'submit', array(
                1
            ));
        }
        $article = null;
