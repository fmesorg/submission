...
{if count($formLocales) > 1 && !$existingUser}
	<tr>
		<td width="10%" class="label">{fieldLabel name="formLocale" key="form.formLanguage"}</td>
...
{if !$implicitAuth}
	<tr>
		<td width="10%" class="label">{fieldLabel name="username" required="true" key="user.username"}</td>
		<td width="90%" class="value"><input type="text" name="username" value="{$username|escape}" id="username" size="20" maxlength="32" class="textField" /></td>
	</tr>
	{if !$existingUser}
	<tr>
		<td></td>
	...
	{if !$existingUser}
		...
		<tr>
        	<td></td>
        	<td class="initials-txt">{translate key="user.initialsExample"}</td>
        </tr>
		<tr>
			<td class="label">{fieldLabel name="gender-m" key="user.gender"}</td>
			<td class="value">
				<select name="gender" id="gender" size="1" class="selectMenu  gender-txt">
		...
		<tr>
			...
			<td class="value">
				<textarea id="affiliation" name="affiliation[{$formLocale|escape}]" rows="5" cols="40" class="textArea reg-field-txt">{$affiliation[$formLocale]|escape}</textarea>
        ...
        <tr>
        	<td></td>
            <td class="instruct">{translate key="user.affiliation.description"}</td>
        </tr>
		<tr>
			<td class="label">{fieldLabel name="signature" key="user.signature"}</td>
			<td class="value"><textarea name="signature[{$formLocale|escape}]" id="signature" rows="5" cols="40" class="textArea reg-field-txt">{$signature[$formLocale]|escape}</textarea><br/><span class="instruct">Please enter a text signature ( eg. Joan Alice Smith, Researcher, Simon Fraser University )</span></td>
		</tr>
        ...
        <tr>
        	<td></td>
            <td class="instruct">{if $privacyStatement}<a class="action" href="#privacyStatement">{translate key="user.register.privacyStatement"}</a>{/if}</td>
        </tr>
		...
		<tr>
			...
			<td class="value">
				<select name="country" id="country" class="selectMenu reg-field-txt">
					<option value=""></option>
		...
		<tr>
			<td class="label">{fieldLabel name="biography" key="user.biography"}<br /><span class="initials-txt">{translate key="user.biography.description"}</span></td>
...
{if $allowRegReader || $allowRegReader === null || $allowRegAuthor || $allowRegAuthor === null || $allowRegReviewer || $allowRegReviewer === null || ($currentJournal && $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $enableOpenAccessNotification)}
	<tr>
		...
		<td class="value"><p class="register-spn">{if $allowRegReader || $allowRegReader === null}<input type="checkbox" name="registerAsReader" id="registerAsReader" value="1"{if $registerAsReader} checked="checked"{/if} /> <label for="registerAsReader">{translate key="user.role.reader"}</label>: <span class="initials-txt">{translate key="user.register.readerDescription"}</span> </p>{/if}
		{if $currentJournal && $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $enableOpenAccessNotification}<input type="checkbox" name="openAccessNotification" id="openAccessNotification" value="1"{if $openAccessNotification} checked="checked"{/if} /> <label for="openAccessNotification">{translate key="user.role.reader"}</label>: {translate key="user.register.openAccessNotificationDescription"}{/if}
		<p class="register-spn">{if $allowRegAuthor || $allowRegAuthor === null}<input type="checkbox" name="registerAsAuthor" id="registerAsAuthor" value="1"{if $registerAsAuthor} checked="checked"{/if} /> <label for="registerAsAuthor">{translate key="user.role.author"}</label>:  <span class="initials-txt">{translate key="user.register.authorDescription"}</span></p>{/if}
		<p class="register-spn">{if $allowRegReviewer || $allowRegReviewer === null}<input type="checkbox" name="registerAsReviewer" id="registerAsReviewer" value="1"{if $registerAsReviewer} checked="checked"{/if} /> <label for="registerAsReviewer">{translate key="user.role.reviewer"}</label>: {if $existingUser} <span class="initials-txt">{translate key="user.register.reviewerDescriptionNoInterests"}</span>{else} <span class="initials-txt">{translate key="user.register.reviewerDescription"}</span></p>{/if}
...
<p class="regi-button"><input type="submit" value="{translate key="user.register"}" class="button defaultButton" /> <input type="button" value="{translate key="common.cancel"}" class="button defaultButton" onclick="document.location.href='{url page="index" escape=false}'" /></p>

{if ! $implicitAuth}
	<p class="regi-button"><span class="formRequired initials-txt">{translate key="common.requiredField"}</span></p>
{/if}{* !$implicitAuth *}

<div id="privacyStatement">
{if $privacyStatement}
	<h3><b>{translate key="user.register.privacyStatement"}</b></h3>
