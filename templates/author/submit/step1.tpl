...
{*<form id="submit" method="post" action="{url op="saveSubmit" path=$submitStep}" onsubmit="return show_submission_closed_modal()">*}
...
<div><span style="margin-right: 20px;">{fieldLabel name="sectionId" required="true" key="section.section"}</span> <select name="sectionId" id="sectionId" size="1" class="selectMenu">{html_options options=$sectionOptions selected=$sectionId}</select></div>

<div style="margin-top:20px;">
<strong>NOTE:</strong> We may assign the submission to another section if we consider it more appropriate.
</div>
...
{foreach name=checklist from=$currentJournal->getLocalizedSetting('submissionChecklist') key=checklistId item=checklistItem}
	{if $checklistItem.content}
		...
		{/if}
		<tr valign="top">
			<td width="3%"><input type="checkbox" id="checklist-{$smarty.foreach.checklist.iteration}" name="checklist[]" value="{$checklistId|escape}"{if $articleId || $submissionChecklist} checked="checked"{/if} /></td>
