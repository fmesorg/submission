	...
	<form method="get" id="searchForm" action="{url op="search"}">
			...
			<tr valign="top">
				...
				<td class="value" colspan="2">
					...
					{if empty($queryFilter)}
						<input type="text" id="query" name="query" size="40" placeholder="{translate key="search.searchAllCategories"}" maxlength="255" value="{$query|escape}" class="textField" />
					{else}
						{$queryFilter}
					{/if}
					&nbsp;
		...
		{if $hasEmptyFilters}
			{capture assign="emptyFilters"}
				...
				<p><input type="submit" value="{translate key="common.search"}" style="margin-top: 15px; width: auto;" class="button defaultButton" /></p>
			{/capture}
...
<h4 style="margin-top: 25px !important;padding: 5px 15px;
background: #575757; text-transform: uppercase !important; font-size: 16px !important;
color: #FFF;">Search Results</h4>
...
<div id="results">
	<table width="100%" class="listing">
		...
		{else}
			<tr>
				<td {if !$currentJournal}colspan="2" {/if}align="left">{$pagination} {*{page_info iterator=$results}*}</td>
