...
<tr valign="top">
	...
	{*<td class="tocPages"><div><div><a class="tocPages-pg">{$article->getPages()|escape}</a></div>{if !$hasAccess || $hasAbstract}<div><a class="tocAbstract" href="{url page="article" op="view" path=$articlePath}">Abstract</a></div>{/if}</div></td>
	...
	<td class="tocPages">		
		<a class="tocPages-pg">{$article->getPages()|escape}</a>
		{if !$hasAccess || $hasAbstract}<a class="tocAbstract" href="{url page="article" op="view" path=$articlePath}">Abstract</a>{/if}
		{if $hasAccess || ($subscriptionRequired && $showGalleyLinks)}
			{foreach from=$article->getGalleys() item=galley name=galleyList}
				<a class="tocGalleyFile" href="{url page="article" op="view" path=$articlePath|to_array:$galley->getBestGalleyId($currentJournal)}" class="file">{$galley->getGalleyLabel()|escape}</a>
				{if $subscriptionRequired && $showGalleyLinks && $restrictOnlyPdf}
					{if $article->getAccessStatus() == $smarty.const.ARTICLE_ACCESS_OPEN || !$galley->isPdfGalley()}	
						<img class="accessLogo" src="{$baseUrl}/lib/pkp/templates/images/icons/fulltext_open_medium.gif" alt="{translate key="article.accessLogoOpen.altText"}" />
					{else}
...
<div class="row">
	<div class="col-md-8 article-section-title-author">
		<div class="row">
			<div class="col-md-12">
				...
				{if !$hasAccess || $hasAbstract}<a class="section-article-title" href="{url page="article" op="view" path=$articlePath}">{$article->getLocalizedTitle()|strip_unsafe_html}</a>{else}{$article->getLocalizedTitle()|strip_unsafe_html}{/if}
			</div>
		</div>
		<div class="row">
			<div class="col-md-12">
	...
	<div class="col-md-4 article-galley-file">
		...
		{if $hasAccess || ($subscriptionRequired && $showGalleyLinks)}
			{foreach from=$article->getGalleys() item=galley name=galleyList}
				{*<a class="tocGalleyFile" href="{url page="article" op="view" path=$articlePath|to_array:$galley->getBestGalleyId($currentJournal)}" class="file">{$galley->getGalleyLabel()|escape}</a>*}
                                <a class="tocGalleyFile" href="{url page="article" op="view" path=$articlePath|to_array:$galley->getBestGalleyId($currentJournal)}" class="file">{if $galley->getGalleyLabel()|escape=="HTML"}Full text{else}{$galley->getGalleyLabel()|escape}{/if}</a>
				{if $subscriptionRequired && $showGalleyLinks && $restrictOnlyPdf}
					{if $article->getAccessStatus() == $smarty.const.ARTICLE_ACCESS_OPEN || !$galley->isPdfGalley()}	
						<img class="accessLogo" src="{$baseUrl}/lib/pkp/templates/images/icons/fulltext_open_medium.gif" alt="{translate key="article.accessLogoOpen.altText"}" />
					{else}
