    ...
    function view($args, &$request)
    {
        ...
        if ($this->_getAnnouncementsEnabled($request) && $this->_announcementIsValid($request, $announcementId)) {
            ...
            if ($announcement->getDateExpire() == null || strtotime($announcement->getDateExpire()) > time()) {
                ...
                if ($announcement->getTypeId() == null || $announcement->getTypeId() == 0) {
                ...
                $templateMgr->append('pageHierarchy', array(
                    $request->url(null, 'announcement'),
    ...
    */
    function setupTemplate($request, $subclass = false)
    {
        ...
        $templateMgr->assign('pageHierachy', array(
            array(
                $request->url(null, null, 'announcements'),
                'announcement.announcements'
            )
        ));
    }
